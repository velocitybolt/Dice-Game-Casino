...
def deploy_contract (contract, abi, account):
    # Fails without abi
    ctrct = web3.eth.contract (abi = abi, bytecode = contract["bin"])
    transaction_hash = ctrct.deploy (
        transaction = {
            "from": account
        },
        args = None
    ...
    print ("contract address = {:s}".format (contract_address))
...
contract_address = deploy_contract (contract_data, abi, web3.eth.accounts[0])
